...
public class AddressAttributeTypeHistoryBuilder extends HistoryBuilderBase<AddressAttributeType> {
	...
	protected void doDiff(@Nullable AddressAttributeType t1, @NotNull AddressAttributeType t2, @NotNull Diff diff) {
		...
		for (Language lang : langs) {
			AddressAttributeTypeTranslation tr1 = t1 != null ? t1.getTranslation(lang) : null;
			AddressAttributeTypeTranslation tr2 = t2.getTranslation(lang);
	...
	public void patch(@NotNull AddressAttributeType t, @NotNull Diff diff) {
		...
		for (HistoryRecord record : diff.getHistoryRecords()) {
			...
			AddressAttributeTypeTranslation tr = t.getTranslation(lang);

			if (tr == null) {
				tr = new AddressAttributeTypeTranslation();
				tr.setLang(lang);
			}
