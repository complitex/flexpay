...
import org.flexpay.accounting.persistence.DocumentStatus;
import org.flexpay.accounting.persistence.EircSubject;
import org.flexpay.accounting.persistence.DocumentTypes;
...
public class QuittancePaymentServiceImpl implements QuittancePaymentService {
	...
	public CashPaymentOperation fromPayment(QuittancePayment payment) throws FlexPayExceptionContainer {
		...
		op.setCreatorOrganization(ApplicationConfig.getSelfOrganization());
		...
		for (QuittanceDetailsPayment qdPayment : payment.getDetailsPayments()) {
			...
			doc.setDocumentType(documentTypeService.read(DocumentTypes.TYPE_CASH_PAYMENT));
			doc.setSumm(qdPayment.getAmount());

			EircSubject debet = new EircSubject();
			debet.setOrganization(ApplicationConfig.getSelfOrganization());
