...
alter table eirc_account_records_tbl
	drop index FK_eirc_account_record_organization,
	drop foreign key FK_eirc_account_record_organization;
...
-- Update service organization for two houses
SELECT @so_grad_n:=e.id FROM flexpay_db.eirc_service_organizations_tbl e, flexpay_db.eirc_service_organization_descriptions_tbl ed
WHERE ed.service_organization_id=e.id AND ed.`language_id`=1 AND ed.`name` = 'ГРАД-Н';
...
set b.eirc_service_organization_id = @so_grad_n;

SELECT @so_uchastok_49:=e.id FROM flexpay_db.eirc_service_organizations_tbl e, flexpay_db.eirc_service_organization_descriptions_tbl ed
WHERE ed.service_organization_id=e.id AND ed.`language_id`=1 AND ed.`name` = 'УЧАСТОК № 49';
...
set b.eirc_service_organization_id = @so_uchastok_49;
