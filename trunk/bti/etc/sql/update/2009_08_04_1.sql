...
update bti_building_attributes_tbl set
...
insert into bti_building_attributes_tbl (building_id, attribute_type_id, value_type, string_value, begin_date, end_date, temporal_flag, discriminator)
	(select a.building_id, a.attribute_type_id, 4, v.attribute_value, v.begin_date, v.end_date, 1, 'hz'
	from bti_building_attributes_tbl a
			left outer join bti_building_attribute_temp_values_tbl v on v.attribute_id=a.id
	where a.discriminator='tmp');

delete from bti_building_attributes_tbl where discriminator='tmp';

alter table bti_building_attributes_tbl
	drop column discriminator,
	drop column normal_attribute_value;

drop table bti_building_attribute_temp_values_tbl;

update common_version_tbl set last_modified_date='2009-08-04', date_version=1;
