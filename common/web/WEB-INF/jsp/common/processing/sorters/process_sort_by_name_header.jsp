...
<input type="submit" class="btn-link" onclick="<s:if test="processSorterByName.activated">$('#processSorterByNameOrder').val('<s:property value="%{processSorterByName.oppositeOrder}" />');</s:if>FP.activateSorter('processSorterByNameActive');" value="<s:text name="common.processing.process.name"/>"/>
